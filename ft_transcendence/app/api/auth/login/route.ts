  ...
  try {
    ...
    if (!zresult.success) {
      return Response.json(
        { error: zresult.error.issues[0].message },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "your are not registed by credintials please user github or other providers",
        },
        { status: 401 },
      );
    }
    ...
    const response = NextResponse.json(
      { success: true, message: "you are logged in", token: token },
      { status: 200 },
    );
